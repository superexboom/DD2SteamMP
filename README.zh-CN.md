...
- 敌方赐福在 `ActorInstance.PostCreate -> BossCalculation.RollBossModifier` 的稳定边界上解析，只对 Arena 当前敌方配置 ActorClass 生效；玩家队伍和无关 Actor 不会继承 `run_test_boss_modifier`。原生测试偏好会在启动前清理，并在所有退出路径清理。
